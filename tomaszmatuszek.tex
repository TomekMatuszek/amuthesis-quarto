...
hyperparameter tuning, nested spatial cross-validation and benchmarking
were performed in order to achieve possibly high and accurate
performance measures. Moreover, thermal band's importance on model
...
surface temperature (LST) of the Earth. There is significant interaction
...
models, probably due to its coarser resolution
...
reprojected from WGS 84 coordinate reference system (EPSG:4326) to UTM
...
Data collected during LUCAS field survey performed by Eurostat was
chosen as land cover training set. At the moment of writing, it is the
...
land, bare land, artificial land and water. In the second phase, a
subsample of grid points is selected and then visited by Eurostat
surveyors. They classify each point according to full LUCAS land cover
and land use classification. The survey takes place in the spring and
summer in order to observe chosen places in their high vegetation season
...
performance and choose parameters that train the model best. On tuning
level of every fold in spatial CV process, I examined 10 random
...
original model results. This value is treated as overall variable
...
suggesting that this variable has greater impact on model predictions
than thermal band.
...
thermal band's impact on model results for these areas but because of
...
more in-depth approach. Result land cover map was created from imagery
of only one 16-day interval and its accuracy was rather average,
especially when compared with state-of-the-art works in this field
...
had significantly greater impact on predicting artificial land than any
other land cover class. After adding spatial context to importance
...
maps will help in better management of growth of metropolitan areas and
in quantifying impact of urbanization on natural environment more
precisely.
