...
Parameters' search spaces and tuning results can be found in Table [-@tbl-tabela3].
...
In *n*-fold cross-validation, every data point is used in both training and testing set.
Whole dataset is randomly divided into *n* equal parts (*folds*).
Then, machine learning model is independently trained *n* times and in each run, different part of the dataset is used as validation set while remaining *n - 1* parts are used to fit the model.
...
Each fold created in the spatial CV is further divided into next *k* folds which comprise the tuning level of the process.
...
The best of 10 (????) hyperparameter combinations is chosen to train the model on outer fold on performance estimation level [@schratz_hyperparameter_2019].
Whole process is then repeated on every of *n* outer folds which leads to most accurate performance measurement as well as defining the best hyperparameter setting.
FIGURE?
...
This study is focused on the latter purpose since my aim was to check if thermal information has an impact on land-cover classification.

Importance of model variables can be measured on two levels: dataset and instance level [@biecek_explanatory_2021].
On the dataset level, we can measure change in model's accuracy depending on presence of one specific variable (Section [-@sec-importance-dataset]).
This gives basic knowledge about this variable's impact on model predictions.
Assessing importance on instance level helps to understand impact of variables for one specific data point (Section [-@sec-importance-instance]).
Moreover, instance-level importance can be utilized to interpolate variable importance values into continuous raster data (Section [-@sec-importance-interpolation]).

### Dataset level {#sec-importance-dataset}

Measuring variable importance on dataset level requires evaluating model twice: once with original data and once with permuted variables of analysed variable.
...
Second method to measure variable importance in machine learning model is instance level evaluation.
...
One way of calculating variable impact on observation result is creating break-down plot (Figure [-@fig-rycina3]).
...
Main idea of this approach is to apply 10 different variable orderings, create a break-down plot for each of them and calculate the mean value of contribution for each variable.
...
Such result enables us to add spatial context to the variable importance, which is further described in Section [-@sec-importance-interpolation].

### Interpolating variable importance {#sec-importance-interpolation}
