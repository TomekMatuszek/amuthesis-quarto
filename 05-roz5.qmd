...
Before moving to thermal band's importance assessment, I explored Landsat dataset more carefully in order to determine correlated variables and interactions between them ([@fig-rycina19]).
Some bands are highly correlated - especially bands from visible spectrum and SWIR bands.
This may suggest that these variables depend on other variable's values [@biecek_explanatory_2021], thus absence of one variable might not lower model's performance because other variable can fill this information gap.
On the other hand, feature selection performed with the help of *mlr3* framework discovered that including all variables still proves to achieve the best model performance.
...
#| label: fig-rycina19
#| echo: false
#| fig-cap: "Correlation matrix of Landsat bands created with the help of *corrplot* package."
#| out-width: "428px"
#| out-height: 400px
...
In the last step of evaluating thermal band's importance for our model, I created partial-dependence (PD) profiles for this variable ([@fig-rycina15]) and compared it with PD profile for near-infrared band (B5) presented in [@fig-rycina16].
Thanks to this method, I checked how probability for choosing certain class changed with increasing values of analysed variables.
...
This allows us to conclude that thermal band might not have significant impact on model's results.
In contrast, in [@fig-rycina16] with B5 variable profile, there are clearly visible trends for nearly every class.
Probabilities change significantly along with changes in near-infrared values, thus suggesting that this variable has greater impact on model's predictions than thermal band.
...
This step created opportunity to examine approximate spatial distribution of thermal band's importance across Poznań metropolitan area ([@fig-rycina17]).
...
In this method, original satellite data was aggregated (resampled) to 1,5 km resolution in order to make analysis more general and shorten the computation time.
...
Correlation of thermal band's importance with artificial land class is visible on two maps.
In both of them, high importance values are concentrated mainly in urban areas, especially in Poznań as it is the biggest city in the study area.
For smaller towns, there is also higher thermal band's impact on model's results for these areas but because of their small size, it is sometimes harder to capture.
