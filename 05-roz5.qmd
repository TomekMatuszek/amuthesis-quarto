...
On the other hand, thermal band is one of four bands, which impact for predicting artificial land was higher than the average.
...
In contrast, B5 variable profile has clearly visible trends for nearly every class ([@fig-rycina16]).
...
Also in smaller towns there is higher thermal band's impact on model results, but because of their size, it is often harder to detect.
